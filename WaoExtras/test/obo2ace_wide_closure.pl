...
# obo2ace_wide_closure.pl reads an obo file and sends ace format output to STDOUT, closure is not restricted to is_a and partof.
# author: Raymond Lee
...
#	    if ($relationship eq "part_of") {push (@good_ids, $related_id);}
	    if ($relationship) {push (@good_ids, $related_id);}
        }
	elsif ($tag eq 'def') {
